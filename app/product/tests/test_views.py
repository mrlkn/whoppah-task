...
class CategoryViewSetTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.category = Category.objects.create(title='Electronics', created_by=self.user)
...
class ProductViewSetTest(APITestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(username='testuser', password='testpassword')
        self.category = Category.objects.create(title='Electronics', created_by=self.user)
        self.product_data = {'title': 'Laptop', 'category': self.category.uuid, 'price': 1.25}
        self.admin_user = User.objects.create_user(username='admin', password='admin', is_staff=True)
    ...
    def test_only_accepted_products_are_listed(self):
        Product.objects.create(title='Apple pro vision xd', category=self.category, created_by=self.user, price=1.25)

        self.client.login(username='testuser', password='testpassword')

        accepted_product = Product.objects.create(
            title='Tablet', category=self.category, created_by=self.user, price=1.25, state=choices.ACCEPTED
        )
    ...
    def test_authorized_user_can_update_product_state(self):
        product = Product.objects.create(title='iphone pro x max ultra diamond gold', category=self.category, created_by=self.user, price=1.25)
    ...
    def test_unauthorized_user_cannot_update_product_state(self):
        product = Product.objects.create(title='Smartphone', category=self.category, price=1.25, created_by=self.admin_user)
